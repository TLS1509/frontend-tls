/** @type {import('tailwindcss').Config} */
export default {
  content: ['./index.html', './src/**/*.{js,ts,jsx,tsx}'],
  theme: {
    extend: {
      colors: {
        primary: {
          50: '#E8F5F9',
          100: '#D0E8F0',
          200: '#A2D1E8',
          300: '#73BAE0',
          400: '#64B1DB',
          500: '#55A1B4',
          600: '#3D7A8A',
          700: '#2D5A66',
          800: '#1D3A42',
          900: '#1A3A42',
        },
        secondary: {
          50: '#FEF4EA',
          100: '#FEE8D5',
          200: '#FDD1A3',
          300: '#FCBB7A',
          400: '#FCA95F',
          500: '#ED843A',
          600: '#D6682A',
          700: '#B84E1F',
          800: '#9A3414',
          900: '#7C1A0A',
        },
        accent: {
          50: '#FFFAED',
          100: '#FFF3DB',
          200: '#FFE8B8',
          300: '#FFDD95',
          400: '#FCD272',
          500: '#F8B044',
          600: '#E69A2F',
          700: '#D4841A',
          800: '#B86F0B',
          900: '#9C5A00',
        },
        neutral: {
          0: '#ffffff',
          50: '#f9fafb',
          100: '#f3f4f6',
          200: '#e5e7eb',
          300: '#d1d5db',
          400: '#9ca3af',
          500: '#6b7280',
          600: '#4b5563',
          700: '#374151',
          800: '#1f2937',
          900: '#111827',
          950: '#0f172a',
        },
        ink: {
          0: '#ffffff',
          50: '#f9fafb',
          100: '#f3f4f6',
          200: '#e5e7eb',
          300: '#d1d5db',
          400: '#9ca3af',
          500: '#6b7280',
          600: '#4b5563',
          700: '#374151',
          800: '#1f2937',
          900: '#1a1a1a',
          950: '#0f172a',
        },
      },
      spacing: {
        xs: '0.5rem',
        sm: '1rem',
        md: '1.5rem',
        lg: '2rem',
        xl: '3rem',
        '2xl': '4rem',
      },
      fontFamily: {
        display: ['League Spartan', 'Helvetica Neue', 'Arial', 'sans-serif'],
        body: ['Nunito', '-apple-system', 'BlinkMacSystemFont', 'Segoe UI', 'system-ui', 'sans-serif'],
        mono: ['JetBrains Mono', 'ui-monospace', 'SFMono-Regular', 'Menlo', 'monospace'],
      },
      fontSize: {
        'heading-1': ['3rem', { lineHeight: '3.5rem', fontWeight: '600' }],
        'heading-2': ['2.25rem', { lineHeight: '2.75rem', fontWeight: '600' }],
        'heading-3': ['1.875rem', { lineHeight: '2.25rem', fontWeight: '600' }],
        'heading-4': ['1.5rem', { lineHeight: '2rem', fontWeight: '600' }],
        'heading-5': ['1.25rem', { lineHeight: '1.75rem', fontWeight: '600' }],
        'h0':        ['3.25rem', { lineHeight: '3.75rem', fontWeight: '800', letterSpacing: '-0.03em' }],
        'h1':        ['2.25rem', { lineHeight: '2.75rem', fontWeight: '700' }],
        'h2':        ['1.75rem', { lineHeight: '2.25rem', fontWeight: '700' }],
        'h2-sm':     ['1.5rem',  { lineHeight: '2rem',    fontWeight: '700', letterSpacing: '-0.02em' }],
        'h3':        ['1.375rem', { lineHeight: '1.875rem', fontWeight: '600' }],
        'h3-lg':     ['1.25rem', { lineHeight: '1.75rem', fontWeight: '600', letterSpacing: '-0.01em' }],
        'h4':        ['1.125rem', { lineHeight: '1.625rem', fontWeight: '600' }],
        'display-sm':['2.5rem',  { lineHeight: '3rem',    fontWeight: '800', letterSpacing: '-0.03em' }],
        'body-label':['1.25rem', { lineHeight: '1.75rem', fontWeight: '700' }],
        'body-lg': ['1.125rem', { lineHeight: '1.75rem' }],
        'body': ['1rem', { lineHeight: '1.5rem' }],
        'body-sm': ['0.9375rem', { lineHeight: '1.5rem' }],
        'caption': ['0.8125rem', { lineHeight: '1.25rem' }],
        'micro': ['0.6875rem', { lineHeight: '1rem' }],
      },
      boxShadow: {
        xs: '0 1px 2px 0 rgba(0, 0, 0, 0.05)',
        sm: '0 1px 3px 0 rgba(0, 0, 0, 0.1), 0 1px 2px -1px rgba(0, 0, 0, 0.1)',
        md: '0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -2px rgba(0, 0, 0, 0.1)',
        lg: '0 10px 15px -3px rgba(0, 0, 0, 0.1), 0 4px 6px -4px rgba(0, 0, 0, 0.1)',
      },
      borderRadius: {
        sm: '4px',
        md: '6px',
        lg: '8px',
        xl: '12px',
        '2xl': '16px',
      },
      opacity: {
        disabled: '0.5',
      },
    },
  },
  plugins: [],
}
