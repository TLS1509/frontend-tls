...
- **Interligne : le pas le porte, ne pas l'écrire à côté.** Chaque `text-*` déclare son `--line-height` ; un `leading-*` posé à côté l'écrase (il gagne quel que soit l'ordre). Le 2026-09-23, 139 surcharges qui bougeaient la ligne de ≤ 1,5 px ont été retirées (`text-body leading-relaxed` = 26 px dans les deux cas). ✅ **Arbitrage n°11 (2026-09-23, option C) : `leading-snug` est réservé aux titres courts en gras** (`font-semibold` et au-delà) — un titre se lit d'un bloc, comme chez Apple et Material. Le texte qu'on lit sur plusieurs lignes (sous-titres, descriptions, légendes) garde l'interligne de son pas : 40 `leading-snug` y ont été retirés, 25 gardés sur des titres. Le détecteur admet `leading-snug` + graisse ≥ 600 et signale le reste.
- **Graisse : un seul poids de titre dans l'app, 700** — celui des tokens h1…h4. ✅ Arbitrage n°12 (2026-09-23, option B) : le titre d'ouverture de page (`PageHero`) se distingue par sa taille (36 px contre 28), pas par sa graisse ; il était forcé à 800 sur plus de 100 pages, comme les chiffres du podium, de la facturation et du compte rendu de coaching. Tous rendus au 700. Le registre 800 reste celui du **site** (`text-hero`, `text-section`), qui a sa propre échelle d'affiche. Ne pas écrire `font-extrabold` ni `font-black` à côté d'un `text-h*` dans l'app.
...
- **n°23 · Voix : « vous » pour le pilotage** (manager, entreprise, admin, CLO),
  « tu » pour l'apprenant ; la **Veille** vouvoie, publications comme écrans
  (surface éditoriale, Chloé, 24/09). Conforme à PRODUCT.md (§ Voix), qui met aussi en
