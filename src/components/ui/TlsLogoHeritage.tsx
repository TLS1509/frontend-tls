/**
 * TlsLogoHeritage — Heritage+ mark (glass/frost/3D) × 5 layouts
 *
 * La mark originale TLS modernisée : gradient radial profond, specular sweep glass,
 * frost rim light, dots sphériques 3D avec highlights. Shadow multi-pass.
 *
 * Layouts : mark · stacked · h1 · h2 · h3
 * Usage   : <TlsLogoHeritage layout="stacked" size={160} />
 */
...
interface TlsLogoHeritageProps {
  layout?: LogoLayout;
  /** Hauteur en px — la largeur est déduite du ratio layout */
  size?: number;
  className?: string;
}

// ── Mark paths (viewBox 0 0 439 402) ──────────────────────────────────────────
...
// ── Layout tables (calquées sur TlsLogoModernized) ────────────────────────────
...
// [x, y, w, h] du mark dans le viewBox du layout
...
const T = {
  the:   '#73AFBF', // primary-400
  learn: '#2F5F6A', // primary-800
  soc:   '#4A8FA1', // primary-600
  bar:   '#EB7724', // amber accent
};

// ── Composant ─────────────────────────────────────────────────────────────────
export function TlsLogoHeritage({
  layout = 'mark',
  size,
  className,
}: TlsLogoHeritageProps) {
  const u = 'tlsh' + useId().replace(/[^a-zA-Z0-9]/g, '');
    ...
    >
      <defs>
        ...
        <radialGradient id={`${u}bg`} cx="27%" cy="23%" r="82%" gradientUnits="objectBoundingBox">
          ...
          <stop offset="18%"  stopColor="#73AFBF" />
          <stop offset="52%"  stopColor="#4A8FA1" />
        ...
        <radialGradient id={`${u}rm`} cx="76%" cy="80%" r="38%" gradientUnits="objectBoundingBox">
          <stop offset="0%"   stopColor="#96C3CF" stopOpacity="0.42" />
          <stop offset="100%" stopColor="#96C3CF" stopOpacity="0" />
        </radialGradient>

        {/* Centre perle */}
        <radialGradient id={`${u}ci`} cx="32%" cy="27%" r="70%" gradientUnits="objectBoundingBox">
          <stop offset="0%"   stopColor="white" />
          <stop offset="20%"  stopColor="#DCEBEF" />
          <stop offset="58%"  stopColor="#B9D7DF" />
        ...
        <filter id={`${u}sh`} x="-30%" y="-24%" width="162%" height="168%">
          ...
          <feFlood floodColor="#1F3E45" floodOpacity="0.26" result="f1"/>
          ...
          <feFlood floodColor="#2F5F6A" floodOpacity="0.20" result="f2"/>
        ...
        <filter id={`${u}ds`} x="-48%" y="-42%" width="200%" height="200%">
          ...
          <feFlood floodColor="#1F3E45" floodOpacity="0.26" result="f"/>
      ...
      {/* ── Mark Heritage+ glass dans sa position layout ── */}
      <svg x={mx} y={my} width={mw} height={mh} viewBox="0 0 439 402" fill="none" overflow="visible">
        {/* Couche 0 : halo teal ambiant */}
        <path d={PM} fill="#55A1B4" opacity="0.30" filter={`url(#${u}gw)`} />

        {/* Couche 1 : corps + shadow */}
        <g filter={`url(#${u}sh)`}>
          <path d={PM} fill={`url(#${u}bg)`} />
        </g>

        {/* Couche 2 : glass specular */}
      ...
      {layout === 'stacked' && <>
        <text x="256" y="272" textAnchor="middle"
          fontFamily={FF} fontSize="20" fontWeight="400" fill={T.the} letterSpacing="2.5">
          THE
        </text>
        <rect x="198" y="282" width="116" height="2.5" rx="1.25" fill={T.bar} opacity="0.7" />
        <text x="256" y="372" textAnchor="middle"
          fontFamily={FF} fontSize="78" fontWeight="800" fill={T.learn} letterSpacing="-1">
          LEARNING
        </text>
        <text x="256" y="432" textAnchor="middle"
          fontFamily={FF} fontSize="34" fontWeight="600" fill={T.soc}>
          Society
        </text>
      </>}

      {/* ── Texte h1 — 3 mots empilés droite ── */}
      {layout === 'h1' && <>
        <text x="134" y="54"
          fontFamily={FF} fontSize="19" fontWeight="400" fill={T.the} letterSpacing="2.8">
          THE
        </text>
        <text x="134" y="111"
          fontFamily={FF} fontSize="56" fontWeight="800" fill={T.learn} letterSpacing="-0.5">
          LEARNING
        </text>
        <text x="134" y="141"
          fontFamily={FF} fontSize="27" fontWeight="600" fill={T.soc}>
          Society
        </text>
      </>}
      ...
      {layout === 'h2' && <>
        <text y="96" fontFamily={FF} fontSize="44" letterSpacing="-0.5">
          <tspan x="148" fill={T.the} fontWeight="400">THE </tspan>
          <tspan fill={T.learn} fontWeight="800">LEARNING</tspan>
        </text>
        <text x="148" y="154"
          fontFamily={FF} fontSize="30" fontWeight="600" fill={T.soc}>
          Society
        </text>
      </>}

      {/* ── Texte h3 — 3 lignes avec séparateur ── */}
      {layout === 'h3' && <>
        <text x="150" y="88"
          fontFamily={FF} fontSize="19" fontWeight="400" fill={T.the} letterSpacing="2.8">
          THE
        </text>
        <rect x="150" y="98" width="90" height="2" rx="1" fill={T.bar} opacity="0.65" />
        <text x="150" y="170"
          fontFamily={FF} fontSize="64" fontWeight="800" fill={T.learn} letterSpacing="-0.5">
          LEARNING
        </text>
        <text x="150" y="214"
          fontFamily={FF} fontSize="30" fontWeight="600" fill={T.soc}>
