  ...
  return (
    <div className={['flex flex-col gap-section', className].filter(Boolean).join(' ')}>
      ...
      <div className="bg-white rounded-2xl shadow-md border border-ink-100 overflow-hidden">
        ...
        <div className="p-8 flex flex-col gap-stack">
          {/* Step counter */}
          <p className="text-caption font-semibold text-ink-400 uppercase tracking-wide">
          ...
          <div className="flex items-start gap-stack">
            {step.icon && (
              <div className={['w-12 h-12 rounded-xl flex items-center justify-center shrink-0', TONE_ICON_BG[tone]].join(' ')}>
                {step.icon}
              </div>
            )}
            <h2 className="text-h3 font-display font-bold text-ink-900 leading-tight">
              {step.title}
            </h2>
          </div>
