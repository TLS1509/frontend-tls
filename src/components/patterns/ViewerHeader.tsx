...
const NAV_BUTTON_BASE =
  'inline-flex items-center justify-center w-10 h-10 rounded-pill text-ink-700 bg-ink-50 hover:bg-ink-100 transition-colors disabled:opacity-disabled disabled:cursor-not-allowed focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary-500';
...
export const ViewerHeader: React.FC<ViewerHeaderProps> = ({
  ...
  const wrapperClasses = [
    'flex items-center gap-stack px-4 sm:px-6 lg:px-8 py-3 bg-white/85 backdrop-blur-glass-light border-b border-ink-200',
    ...
    <header className={wrapperClasses} role="banner">
      ...
      {onBack && (
        <button
          ...
          className="shrink-0 inline-flex items-center gap-2 px-3 py-1.5 rounded-pill bg-ink-50 hover:bg-ink-100 text-ink-700 hover:text-ink-900 transition-colors text-caption font-semibold focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary-500"
        >
      ...
      {(title || eyebrow || subtitle) && (
        <div className="flex-1 min-w-0 flex flex-col items-start sm:items-center">
          {eyebrow && (
            <span className="font-body text-micro font-bold uppercase tracking-wider text-primary-700 truncate w-full text-left sm:text-center">
              {eyebrow}
            </span>
          )}
          {title && (
      ...
      {onClose && (
        <button
          ...
          onClick={onClose}
          aria-label="Fermer"
          className="shrink-0 inline-flex items-center justify-center w-10 h-10 rounded-pill text-ink-700 bg-ink-50 hover:bg-danger-bg hover:text-danger-fg transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary-500"
        >
