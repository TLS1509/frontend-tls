...
### Le bouton — l'API est une grille, pas une liste de noms

> **Posé le 2026-09-17.** `emphasis` (solid · soft · outline · ghost · link) ×
> `tone` (brand · warm · sun · danger · neutral). Les treize `variant`
> historiques sont dépréciés : ce ne sont plus que des coordonnées.

**La règle sous la règle : un nom ne porte jamais de classes.** La bascule du
17/09 a changé ce que rendait `variant="primary"` en modifiant la chaîne de
classes sous ce nom ; la case `solid`/`brand` pointait dessus et a suivi en
silence. Résultat mesuré : le CTA principal de la page d'accueil marketing, qui
...
poser n'importe où. Le jour du renommage, 26 boutons « verre » ont été trouvés
sur des heros clairs — blanc sur blanc, action principale de vingt pages.
