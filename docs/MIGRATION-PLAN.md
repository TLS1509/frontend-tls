...
| 16.16 | #13bis GDPR/AI Act | S (3–5j) | **MVP** | ✅ GdprConsentType/UserGdprConsents/UserAIConsents/DsarRequest types · usePrivacyStore (#15, persisted: gdprConsents + aiConsents + dsarRequests) · ProfileConsent câblé store (AI consents persistés + section GDPR 3 types essential/analytics/marketing + ConsentBanner dismiss persisté) · PrivacyDsar câblé store (DSAR submission persistée + historique depuis store + deadline légale 30j) · **16.13bis (AI Act)** : ChatInterface → AITransparencyLabel (generated, confidence score coloré ≥80/60/< colors, sources pills, privacy block shield) + ChatMessage/ChatFeedback/ChatSourceCitation types · ProfilePrivacy → historique consentements 4 entrées + politique conservation 5 catégories (SectionCard Clock/Database) · CoachLearnerProfile → section "Recommandations IA" (3 recs, confidence badge, AITransparencyLabel recommended, AIOverrideButton requireReason — rejet persisté via useState) |
