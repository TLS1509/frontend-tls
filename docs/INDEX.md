...
| `SOLUTIONS-01-PREUVE.md` | `docs/product/` | **Solutions du défaut 1** (colonne vertébrale de la preuve), 2026-07-23. Premier d'une série (un doc par défaut). 3 options évaluées sous 6 lentilles (apprenance · instructional design · biais/neuro · UX/UI · conformité · faisabilité) : **A** registre à niveau dérivé (léger, mais retire l'agentivité + frôle la décision automatisée) · **B** portfolio co-construit (meilleure apprenance + idéal RGPD art. 22, mais coûteux/coach-dépendant) · **C** assertion+rubrique (le plus valide/vendable B2B, mais lourd + risque scolaire). **Reco tranchée : un registre unique à 3 régimes de preuve gradués selon l'enjeu du palier** — socle technique de A + sémantique d'agentivité de B + rigueur de C réservée aux paliers certifiants. Invariant : l'auto-déclaration n'écrit jamais le niveau (→ `selfAssessedLevel` de calibration). |
| `SOLUTIONS-02-GAMIFICATION.md` | `docs/product/` | **Solutions du défaut 2** (gamification vs attestation), 2026-07-23. 3 options (jeu subordonné/cloisonné · attestation seule · deux monnaies re-motivées par SDT). Reco : **l'attestation est la récompense** (informationnelle, sûre au sens de Deci & Ryan) ; **firewall immédiat** = soustraction (retirer +20 XP sur la réflexion, ne pas coder l'atrophie-badge ni le claim payant, tous spec-only) ; streak recadré non-punitif (Duolingo streak-freeze) réservé au novice ; KPI enterprise présence→compétence démontrée. Un badge atteste un fait daté (permanent) ; le déclin d'une compétence = signal de *fraîcheur* non punitif, jamais un downgrade. |
| `SOLUTIONS-03-VALIDATION-HUMAINE.md` | `docs/product/` | **Solutions du défaut 3** (validation humaine / RGPD art. 22), 2026-07-23. 3 options (attestée · rubrique · collégiale). Reco : **la rubrique (B) est le socle obligatoire de toute montée de niveau** — A seule recrée le défaut sous une trace trompeuse (rubber-stamping = « ne compte pas » au sens art. 22) ; C réservé au jalon certifiant. Règle d'or UX : **rationale affiché AVANT le bouton**, taux de contradiction suivi comme indicateur de *conformité, jamais de performance*. Résout le `TBD` 04:1492 (« pas d'auto-update — l'attestation signée EST la mise à jour ») → **même objet-preuve que le défaut 1**. Asymétrie : exemption RGPD art. 22 / obligation AI Act art. 14 — renforce sans garantir. |
| `SOLUTIONS-04-IA-CONFIANCE.md` | `docs/product/` | **Solutions du défaut 4** (score de confiance IA), 2026-07-23. 3 options composées. Reco : **aucun `%` à l'écran** — abstention comme état de première classe (« pas trouvé → ton coach ») ; le signal qualitatif (bien ancrée/à recouper/peu ancrée) piloté par un **vérificateur de groundedness** (juge NLI), jamais Mistral se notant lui-même ; **tableau de la ligne de partage** : churn→ML tabulaire+humain décideur (jamais de mail auto), org intelligence→SQL déterministe, forecast→projection stat, adéquation mission→score de recouvrement, niveaux questionnaire→`selfAssessedLevel`. Le LLM ne garde que la reformulation. Ancrages : automation bias (Parasuraman&Riley), confiance appropriée (Lee&See), fausse précision (Moore&Healy), calibration LLM (Kadavath), groundedness (RAGAS). |
| `SOLUTIONS-05-SCOPE-COHERENCE.md` | `docs/product/` | **Solutions du défaut 5** (incohérences + sur-ingénierie), 2026-07-23. 3 postures (réconcilier vers le bas · réécrire un cahier canonique · walking skeleton radical). Reco : **doctrine de C, véhicule de A, refus explicite de B** (le tunnel de re-spécification = le piège temporel). **Gate en 5 critères** avant qu'un cahier consomme une heure de dev : 1 techno tranchée · 1 chiffre d'effort daté · 1 unité de crédit cohérente · 0 fonction hors North Star sans utilisateur réel · le code fait foi. Liste de coupe (09→Email+In-App, 03→un seul rail paiement, 06→rôles built-in) + 3 vagues (décider / squelette de preuve / réconcilier tiré par la demande). Ancrages : dette (Cunningham), YAGNI, walking skeleton (Cockburn), MVP (Ries). |
